...
{	
	...
	if(current->is_policy_on){
		if (current->privilege_level < 2){
			// Add to log
			//Create new info node and linkes it to the list:
			...
			//Add to log list:
			if(current->info_list_size < 0){ //size initialized to be -1
				current->info_list_head = new_info;
				//LIST_HEAD_INIT(&(current->info_list_head->list_ptr));
				LIST_HEAD_INIT(&new_info->list_ptr);
				current->info_list_size = 1;
			}else{
				list_add_tail(&(new_info->list_ptr),&(current->info_list_head->list_ptr));

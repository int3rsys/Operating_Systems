...
int main(){
  ...
  result = enable_policy(pid, 10, 234123);
  result = set_process_capabilities(pid,1,234123);
  pid_t new_pid;
	/* Should invoke error: */
  new_pid = fork();
	result = set_process_capabilities(pid,0,234123);
	/* Should invoke error: */
	result = sched_yield();
	/* Should invoke error: */
	result = wait(w);
	data = malloc(sizeof(struct forbidden_activity_info)*4);
	result = get_process_log(pid, 3, data);
	for(i;i<4;i++){
		printf("Removing req_level %d, proc level %d, created in %d\n", data[i].syscall_req_level, data[i].proc_level, data[i].time);
	}
	free(data);

	result = set_process_capabilities(pid,2,234123);
	new_pid = fork();
	...
	if (new_pid != 0){
		result = set_process_capabilities(pid,1,234123);
		new_pid = fork();
	  result = disable_policy(pid, 234123);
	}
  return 0;
}

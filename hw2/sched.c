...
void __init sched_init(void)
{
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		/* HW2 edit  - we initialize our sc list*/
		rq->sc = rq->arrays + 2;
		spin_lock_init(&rq->lock);
		INIT_LIST_HEAD(&rq->migration_queue);
		// HW2 edit: j changed from 2 => 3
		for (j = 0; j < 3; j++) {
			array = rq->arrays + j;
			...
			// HW2 edit: if j==2, then it's sc, we want to initialize only the first list_entry
			if(j==2){
				INIT_LIST_HEAD(array->queue);
				// HW2 edit: Added else case below
			}else{
